...
float aA=0.05;    //acceleració poma
float agV=0;       //velocitat poma groga
float agA=0.06;    //acceleració poma groga
float abV=0;       //velocitat poma blava
float abA=0.08;    //acceleració poma blava
int p=0;          //puntuació
...
void draw(){
 ...
 if(aY>height){
  ...
  factor=int(random(1,10));
  
  if(factor==1){
  aY=20;
  aX= int(random(25,(width-5)));
  aV=0;
  pC=true;
  }
}
 ...
 if(agY>height){
  ...
  if(factor==2){
  agY=20;
  agX= (int(random(25,(width-5)))+vaiven);
  agV=0;
  pC=true;
}
 }
